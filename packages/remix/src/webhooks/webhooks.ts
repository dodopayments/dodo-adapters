...
export const Webhooks = ({
  ...
  const action = async ({ request }: { request: Request }) => {
    if (request.method !== "POST") {
      throw new Response("Method not allowed. Use POST", { status: 405 });
    }
    ...
    try {
      ...
      throw new Response("Error while verifying webhook", {
        status: 500,
      });
    }
    ...
    if (!success) {
      ...
      throw new Response(`Error parsing webhook payload: ${error.message}`, {
        status: 400,
      });
    }
